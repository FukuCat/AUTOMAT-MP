...
import engine.model.state.MealyState;
...
public class Machine01Scene extends GameScene{
    ...
    public void init() {
        ...
        getInputTimers().put(KeyEvent.VK_A, new SimpleTimer(0.125f));
        getInputTimers().put(KeyEvent.VK_D, new SimpleTimer(0.125f));
        camera.reset();
        getActors().clear();
        getActors().add(oA);
    ...
    public void input(Input input, long deltaTime) {
        ...
        if(getInputTimers().get(KeyEvent.VK_A).checkTime() && input.getKeyboardKey(KeyEvent.VK_A)){
            machine.prevState();
            System.out.println(machine.getOutput());
        }
        if(getInputTimers().get(KeyEvent.VK_D).checkTime() && input.getKeyboardKey(KeyEvent.VK_D)){
            machine.nextState();
            System.out.println(machine.getOutput());
        }
